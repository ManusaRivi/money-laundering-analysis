...
func filterTransactionByAmount(tx protocol.Transaction, operator string, value float64) bool {
	switch operator {
	case ">":
...
func filterTransactionByFormat(tx protocol.Transaction, operator string, values []string) bool {
	if tx.PaymentFormat == "" {
...
func filterTransaction(tx protocol.Transaction, filterType string, operator string, floatValue float64, stringValues []string) bool {
	switch filterType {
	case "amount":
