...
func TestDispatchDropsDuplicates(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		if _, _, err := p.Dispatch(msg, handlers); err != nil {
...
func TestDispatchPerClientIsolation(t *testing.T) {
	...
	if _, _, err := p.Dispatch(msgWith(t, a, idA), handlers); err != nil {
		t.Fatalf("dispatch a: %v", err)
	}
	if _, _, err := p.Dispatch(msgWith(t, b, idB), handlers); err != nil {
